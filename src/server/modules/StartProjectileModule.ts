import type { StartProjectileEvent } from "../Types";
...
export class StartProjectileModule extends Module {
	...
	private _projectileCooldownTime = 0;
	private _blockVehicleWeapons = true;

	public onLoad(): void {
		this._projectileCooldownTime = Config.getValue<number>(
			this.config,
			"projectileCooldown",
		);
		this._blockVehicleWeapons = Config.getValue<boolean>(
			this.config,
			"blockVehicleWeapons",
		);
		this.eventHandler.subscribe(
			"startProjectileEvent",
			this.onProjectileSpam.bind(this),
		);
	}
	...
	private onProjectileSpam(source: number, data: StartProjectileEvent) {
		...
		if (
			this._blockVehicleWeapons &&
			Utility.WEAPONS_VEHICLE.has(data.weaponHash)
		) {
			const violation = new Violation(
				source,
				"Projectile Weapon [C2]",
				this.name,
			);
			violation.banPlayer();
		}
